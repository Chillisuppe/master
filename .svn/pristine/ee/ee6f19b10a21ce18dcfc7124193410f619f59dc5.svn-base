...
!>        for DG discretization in weak form.
!>
MODULE DG_flux
...
!--- element loop for inner flux computation
    ...
    !$OMP r_Q_q, r_rhs, r_eMinvdpsidx, r_eMinvdpsidy) &
    !$OMP SCHEDULE(STATIC)
...
!--- evaluate inner element flux and source term
      CALL elmtflux(r_rhs, i_faceunknowns, i_equadpts, r_eqwei, &
                    r_eMinvdpsidx, r_eMinvdpsidy, r_Q_q)
...
!
  SUBROUTINE elmtflux(r_rhs, i_faceunknowns, i_equadpts, r_eqwei, &
                      r_eMinvdpsidx, r_eMinvdpsidy, r_Q)
...
!--- local declarations
    INTEGER (KIND = GRID_SI)                                :: i_quad, i_dof
    REAL (KIND = GRID_SR), DIMENSION(i_nprogvars)           :: r_Fstar
...
!--- perform edge quadrature
    edge_quad_loop: DO i_quad=1,i_gquadpts
      ...
      edge_dof_loop: DO i_dof=1,i_faceunknowns
        r_rhs_l(:,i_dof) = r_rhs_l(:,i_dof) - r_gqwei(i_quad)*r_gMinvpsi(i_dof,i_quad)*r_Fstar
        r_rhs_r(:,i_dof) = r_rhs_r(:,i_dof) - r_gqwei(i_quad)*r_gMinvpsi(i_dof,i_quad)*r_Fstar
