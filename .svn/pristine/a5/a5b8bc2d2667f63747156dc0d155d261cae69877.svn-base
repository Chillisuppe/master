!*******************************************************************************
!
!> @file  TEST_eqsource_writecoards.f90
!> @brief program to test IO of COARDS-files with the MISC_eqsource modules
!>        (this is part 1 of 2 and writes the netCDF files)
!
!*******************************************************************************
PROGRAM TEST_eqsource_writecoards
...
!--- set FLASH description in global datastruct
  GRID_parameters%program_name = 'TEST_eqsource_writecoards'
...
!--- allocate workspace
  ...
  IF(i_alct /= 0) &
    CALL grid_error(c_error='[TEST_eqsource_writecoards]: Could not allocate workspace')
...
!--- load fault data and compute generated uplift on the grid
  CALL initialize_source('Okada_20041226Sumatra_Hira06.dat',&
                         [90.0_GRID_SR, 98.0_GRID_SR], &
                         [ 0.0_GRID_SR, 16.0_GRID_SR], 321, 641, 80)

  CALL write_uplift_coards('TEST_dz_coards.nc', 0.0_GRID_SR)
  WRITE(*,'(A)') 'Uplift has been written to netCDF file.'
  WRITE(*,*)
...
!#ifdef USE_NETCDF
  CALL ncugrid_createmesh('TEST_eqsource_writecoards.nc', 'Mesh2', i_numelmt*GRID_elementnodes, i_numelmt, &
                          i_elmtnodesdg, r_coonoddg, 'none', 'none')

  CALL ncugrid_putvariable('TEST_eqsource_writecoards.nc', 'Mesh2_disp', 'displacement', 'displacement', 'm', &
                           'Mesh2', 'node', r_vdata, 0.0_GRID_SR)

!#else
!  CALL grid_error(c_error='[TEST_eqsource_writecoards]: NetCDF library not installed - cannot write data!')
!#endif
...
END PROGRAM TEST_eqsource_writecoards
