...
MODULE DG_equation
  ...
  PUBLIC  i_nprogvars, i_nsrcterms, i_valQ, i_valS, &
          VAR2D_MZERO, VAR1D_HZERO, VAR1D_HCNST, VAR1D_CORIO, &
          VAR1D_WINDX, VAR1D_WINDY, VAR1D_PARACHECK, &
...
!--- compute the source terms
      ...
      r_source(2) = &
        + r_iswet * r_hv_q(i_quad) * r_cor_q(i_quad) &
        + r_iswet * r_h_q(i_quad) * r_gradb_q(i_quad,1) &
        - r_iswet * (1-r_heavi)*(r_tau_q(i_quad,1) * p_equationparam%r_gammat / (p_equationparam%r_rho*(r_h_q(i_quad)+r_tol))) &
        + r_iswet * r_manning * r_hu_q(i_quad) &
        + r_iswet * p_equationparam%r_visc*r_laphu_q(i_quad)

      r_source(3) = &
        - r_iswet * r_hu_q(i_quad) * r_cor_q(i_quad) &
        + r_iswet * r_h_q(i_quad) * r_gradb_q(i_quad,2) &
        - r_iswet * (1-r_heavi)*(r_tau_q(i_quad,2) * p_equationparam%r_gammat / (p_equationparam%r_rho*(r_h_q(i_quad)+r_tol))) &
        + r_iswet * r_manning*r_hv_q(i_quad) &
        + r_iswet * p_equationparam%r_visc*r_laphv_q(i_quad)
...
!--- Coriolis
    ...
    p_iovars( 5)%c_long_name     = 'Coriolis forcing'
    p_iovars( 5)%c_standard_name = 'Coriolis forcing' ! not CF conform!
    p_iovars( 5)%c_units         = 'None'

!--- wind in x-direction
    p_iovars( 6)%r_factor        = 1.0_GRID_SR
    p_iovars( 6)%c_varname       = 'wind_x'
