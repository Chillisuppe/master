...
!--- element loop for inner flux computation
    ...
    !$OMP r_Q_q, r_rhs, r_dQdx_q, r_dQdy_q, r_h, r_b, i_imaxh, r_iswet) &
    !$OMP SCHEDULE(STATIC)
...
!--- evaluate inner element flux and source term
      CALL elmtflux(r_rhs, i_faceunknowns, i_equadpts, r_eqwei, r_eMinvpsi, &
                    r_Q_q, r_dQdx_q, r_dQdy_q, r_iswet)
...
!--- compute flux divergence
      r_Div = divflux(r_Q(:,i_quad), r_dQdx(:,i_quad), r_dQdy(:,i_quad), r_iswet)
...
!--- Xing, Zhang & Shu (2010), Xing & Zhang (2013)
          ...
          r_Fleft = r_Fleft + MATMUL(fluxGrav(r_h_l_star) - fluxGrav(r_Q_l(1,i_quad)), r_normal)
          r_Frght = r_Frght + MATMUL(fluxGrav(r_h_r_star) - fluxGrav(r_Q_r(1,i_quad)), r_normal)
        END IF
