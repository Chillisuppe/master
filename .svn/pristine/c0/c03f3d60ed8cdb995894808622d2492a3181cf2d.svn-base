!*******************************************************************************
!
!> @file  DG_initial_freebasin.f90
!> @brief contains module DG_initial
!
!*******************************************************************************
! MODULE DESCRIPTION:
!> @brief initializes fields for long wave resonance in a cirular parabolic basin
!>        (Thacker 1981). Parameters according to Lynett et. al (2002)
...
!
  SUBROUTINE initial_setparam
    ...
    ! distance from center point to shoreline when surface is flat
    p_testparam%treal(1)%c_keyword = 'PARAM_A'
    p_testparam%treal(1)%i_size    = 1

    ! distance from center point to initial shoreline
    p_testparam%treal(2)%c_keyword = 'PARAM_R0'
...
!--- local declarations
    ...
    REAL (KIND = GRID_SR)                                   :: r_r2, r_aux, r_h0, r_a
...
!--- initialize some constants
      ...
      DO i_elmt=1, i_numelmt
        DO i_dof=1, i_faceunknowns
          ...
          r_r2  = r_coodof(1,i_globaldof)**2 + r_coodof(2,i_globaldof)**2
          r_S(1,i_globaldof) = r_h0*(r_r2/r_a**2) * GRID_GRAV
...
!--- local declarations
    REAL (KIND = GRID_SR)                                         :: r_r2oa2, r_D, &
      r_eta, r_AA, r_omega, r_fac1, r_fac2, r_h0, r_a, r_r0

!--- initialize some constants

    r_h0 = p_equationparam%r_depth
    r_a  = p_testparam%treal(1)%p_value(1)
    r_r0 = p_testparam%treal(2)%p_value(1)

    r_AA    = (r_a**4 - r_r0**4) / (r_a**4 + r_r0**4)
    r_omega = SQRT(8.0_GRID_SR*GRID_GRAV*r_h0) / r_a

    r_r2oa2 = (r_coo(1)**2 + r_coo(2)**2) / r_a**2
    r_D     = r_h0*(1.0_GRID_SR - r_r2oa2)
    r_fac1  = 1.0_GRID_SR-r_AA**2
    r_fac2  = 1.0_GRID_SR-r_AA*COS(r_omega*r_time)

    r_sol(1) = MAX(0.0_GRID_SR, r_h0*(SQRT(r_fac1) / r_fac2 - r_r2oa2 * r_fac1 / r_fac2**2))
    r_sol(2) = r_sol(1) * r_omega*r_coo(1)*r_AA*SIN(r_omega*r_time) / (2.0_GRID_SR*r_fac2)
    r_sol(3) = r_sol(1) * r_omega*r_coo(2)*r_AA*SIN(r_omega*r_time) / (2.0_GRID_SR*r_fac2)
