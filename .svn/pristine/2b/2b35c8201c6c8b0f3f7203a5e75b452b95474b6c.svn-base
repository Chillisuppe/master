...
!--- allocate workspace
    ...
    IF (i_alct /= 0) CALL grid_error(c_error='[DG_limiter_KuSV]: Could not allocate enough memory')
...
!--- compute mean, min and max values for each element
    ...
    DO i_node=1, i_numnode
      IF (i_node == i_ndmaster(i_node)) THEN
        ...
        r_emin(5,i_node)  = MINVAL(r_emean(5,i_ndpatch(1:i_loc, i_node)))
        r_emin(6,i_node)  = MINVAL(r_emean(6,i_ndpatch(1:i_loc, i_node)))
        ...
        r_emax(5,i_node)  = MAXVAL(r_emean(5,i_ndpatch(1:i_loc, i_node)))
        r_emax(6,i_node)  = MAXVAL(r_emean(6,i_ndpatch(1:i_loc, i_node)))
      END IF
    END DO
...
!--- limit in surface elevation
      ...
      DO i_node=1,GRID_elementnodes
        ...
        r_hext = r_Q(1,i_elementdofs(i_node,i_elmt))+r_S(1,i_elementdofs(i_node,i_elmt))
        IF (r_hext > r_emean(4,i_elmt)+1e-13) &
          r_alpha = MIN(r_alpha, (r_emax(4,i_elementnodes(i_node,i_elmt)) - r_emean(4,i_elmt)) / (r_hext - r_emean(4,i_elmt)))
        IF (r_hext < r_emean(4,i_elmt)-1e-13) &
          r_alpha = MIN(r_alpha, (r_emean(4,i_elmt) - r_emin(4,i_elementnodes(i_node,i_elmt))) / (r_emean(4,i_elmt) - r_hext))
      END DO
...
!--- compute limited u-velocity
      ...
      r_ulim(1,:) = MAX(MIN(r_utmp, r_emax(5, i_elementnodes(1,i_elmt))), r_emin(5, i_elementnodes(1,i_elmt)))
      r_utmp      = velocity(r_Q(1,i_elementdofs(2,i_elmt)), r_Q(2,i_elementdofs(2,i_elmt)))
      r_ulim(2,:) = MAX(MIN(r_utmp, r_emax(5, i_elementnodes(2,i_elmt))), r_emin(5, i_elementnodes(2,i_elmt)))
      r_utmp      = velocity(r_Q(1,i_elementdofs(3,i_elmt)), r_Q(2,i_elementdofs(3,i_elmt)))
      r_ulim(3,:) = MAX(MIN(r_utmp, r_emax(5, i_elementnodes(3,i_elmt))), r_emin(5, i_elementnodes(3,i_elmt)))
...
!--- compute u-velocities at other nodes based on linear momentum assumption
      ...
      ELSE
        ...
        END IF
      END IF
!       write (*,*) r_emean(2,i_elmt)-SUM(r_Q(2,i_elementdofs(:,i_elmt)))/3.0, &
!                   r_emean(2,i_elmt)-SUM(r_Qlim(1,i_elementdofs(:,i_elmt))*r_ulim(:,1))/3.0, &
!                   r_emean(2,i_elmt)-SUM(r_Qlim(1,i_elementdofs(:,i_elmt))*r_ulim(:,2))/3.0, &
!                   r_emean(2,i_elmt)-SUM(r_Qlim(1,i_elementdofs(:,i_elmt))*r_ulim(:,3))/3.0

!--- compute limited v-velocity
      r_utmp      = velocity(r_Q(1,i_elementdofs(1,i_elmt)), r_Q(3,i_elementdofs(1,i_elmt)))
      r_ulim(1,:) = MAX(MIN(r_utmp, r_emax(6, i_elementnodes(1,i_elmt))), r_emin(6, i_elementnodes(1,i_elmt)))
      r_utmp      = velocity(r_Q(1,i_elementdofs(2,i_elmt)), r_Q(3,i_elementdofs(2,i_elmt)))
      r_ulim(2,:) = MAX(MIN(r_utmp, r_emax(6, i_elementnodes(2,i_elmt))), r_emin(6, i_elementnodes(2,i_elmt)))
      r_utmp      = velocity(r_Q(1,i_elementdofs(3,i_elmt)), r_Q(3,i_elementdofs(3,i_elmt)))
      r_ulim(3,:) = MAX(MIN(r_utmp, r_emax(6, i_elementnodes(3,i_elmt))), r_emin(6, i_elementnodes(3,i_elmt)))
