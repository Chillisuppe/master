!*******************************************************************************
!
!> @file  IO_netcdfplot_dg_elmtmeans.f90
...
!>        In this version the element mean values are computed and written
!>        as cell data.
!>
...
!--- local declarations
    INTEGER (KIND = GRID_SI)                                    :: i_var, i_tcnt, &
      i_alct, i_eunknowns, i_elmt, i_niovars
...
!--- define data variables
    DO i_var = 1, i_niovars
      ...
      p_vdataarr(i_var)%c_location      = 'face'
...
!--- average over each cell
    DO i_elmt = 1, i_numelmt
      DO i_var = 1, i_niovars
        r_vdata(i_var,i_elmt) = SUM( &
          r_tmpdat(i_var,i_elmtdofs(:,i_elmt))) &
          / REAL(i_eunknowns, GRID_SR)
      END DO
    END DO
...
!--- deallocate data arrays
    ...
    END DO
    DEALLOCATE(p_vdataarr, r_tmpdat, r_vdata, i_eltlev)
