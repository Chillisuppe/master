!*******************************************************************************
!
!> @file  DG_initial_beach_reservoir.f90
!> @brief contains module DG_initial
!
!*******************************************************************************
! MODULE DESCRIPTION:
!> @brief initializes a channel with uniformly sloping source_update that contains
!>        a reservoir and a horizontal (total) water height.
!>
!> Initial conditions for quasi 1D Tidal Basin with Bottom Convexity as in
!> A. Balzano: "Evaluation of methods for numerical simulation of wetting and
!> drying in shallow water flow models", Coastal Engineering 34, pp 83-107 (1998)
!> Testcase 5.2
!
MODULE DG_initial
...
!--- initialize some constants
      ...
      r_c1    = 2.608695652_GRID_SR ! 60/23
      r_c2    = 4.347826087_GRID_SR ! 100/23

!--- loop over the elements
      DO i_elmt=1, i_numelmt
        DO i_dof=1, i_faceunknowns
          r_x = r_coodof(1,i_elmtdofs(i_dof,i_elmt))
          IF ((r_x .LE. 3600) .OR. (r_x .GE. 6000)) THEN
            r_S(1,i_elmtdofs(i_dof,i_elmt)) = r_x*r_alpha * GRID_GRAV
          ELSEIF ((r_x.GE.3600).AND.(r_x.LE.4800)) THEN
            r_S(1,i_elmtdofs(i_dof,i_elmt)) = (-r_x*r_alpha + r_c1) * GRID_GRAV
          ELSEIF ((r_x .GE. 4800) .AND. (r_x.LE.6000))THEN
            r_S(1,i_elmtdofs(i_dof,i_elmt)) = (REAL(r_x/920,GRID_SR) - r_c2) * GRID_GRAV
          END IF
        END DO
      END DO
    END IF
...
!--- loop over the DOFs
    dof_loop: DO i_count=1, i_faceunknowns*i_numelmt
      ...
      IF ((r_x .LE. 3600.0) .OR. (r_x .GE. 6000.0)) THEN
        r_S = r_x*r_alpha
      ELSEIF ((r_x.GE.3600.0).AND.(r_x.LE.4800.0)) THEN
        r_S = -r_x*r_alpha+r_c1
      ELSEIF ((r_x.GE.4800.0) .AND. (r_x.LE.6000.0)) THEN
        r_S = REAL(r_x/920.0,GRID_SR) -r_c2
      END IF

      r_H = r_height - r_S

      r_Q(1,i_count) = r_H * GRID_GRAV
