...
MODULE ADV_dg
  ...
  USE DG_initial, ONLY : initialize_testcase, initialvalues_iter, source_update, dg_elmt_solution
  USE DG_timestepping
  USE DG_utils, ONLY : compute_edgeinfo, fvm_createmetrics, Vandermonde2D
  USE MISC_diag
  USE DG_cfl
  USE DG_equation, ONLY : i_nprogvars, i_nsrcterms, i_valQ, i_valS, pre_timestepping
...
!--- local declarations
    ...
    INTEGER (KIND = GRID_SI)                                   :: i_tmp, i_alct, &
      i_degree, i_faceunknowns, i_numdofs, i_numelmt, i_numnode, i_numedge
...
!--- initialize testcase and write initial conditions before the adaptation loop to grid
      CALL initialize_testcase(p_ghand(i_timeplus), p_param, p_timestepinfo%r_modeltime)
      CALL initialvalues_iter(p_ghand(i_timeplus), p_param, p_timestepinfo%r_modeltime)
...
!--- write initial conditions to adapted grid
        CALL initialvalues_iter(p_ghand(i_timeplus), p_param, p_timestepinfo%r_modeltime)
...
!--- local declarations
    ...
    INTEGER (KIND = GRID_SI)                                   :: i_numelmt, i_numedge, i_numnode, &
      i_faceunknowns, i_degree, i_numdofs, i_equadpts, i_gquadpts, i_gpsinonzero
...
!--- read in coefficients for time stepping
    CALL read_coefficients(p_param, r_coeff)
...
!--- call the time evolution computation
        ...
        CALL timestepping(p_ghand(i_timeplus), r_Q, r_Q_next, r_S, &
                          p_timestepinfo%r_modeltime, r_dt, r_coeff, i_numelmt, &
