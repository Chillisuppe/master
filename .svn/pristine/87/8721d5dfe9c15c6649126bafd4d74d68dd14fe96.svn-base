!*******************************************************************************
!
!> @file  DG_error_center_vortex.f90
...
!--- local declarations
    ...
    REAL (KIND = GRID_SR)                                 :: r_vb, r_time, r_dist
...
!--- position of the vortex
    r_time  = p_timestepinfo%r_modeltime
    r_vb    = 1.0
    r_centr = (/ 2.0, 1.0 /)
    r_centr(1) = MOD(r_centr(1) + r_vb*r_time, 4.0)

!--- loop through all elements
    elmt_loop: DO i_elmt=1, i_numelmt

!--- determine L2 distance to position of the vortex at time r_time
      r_xy   = r_coo(:,i_elmtdofs(:, i_elmt))
      r_dist = MAXVAL(ABS( (r_xy(1,:)-r_centr(1))**2 + (r_xy(2,:)-r_centr(2))**2 ))
