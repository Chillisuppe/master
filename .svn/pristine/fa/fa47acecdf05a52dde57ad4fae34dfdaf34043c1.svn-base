...
!
  SUBROUTINE compute_cfl(p_ghand, r_Q, r_S, r_hmin, i_faceunknowns, i_elmtnodes, &
    ...
    p_rteqninfo%r_pressmax = -HUGE(1.0_GRID_SR)
    p_rteqninfo%r_pressmin =  HUGE(1.0_GRID_SR)
...
!--- compute velocity
      ...
      DO i_dof = 1, i_faceunknowns
        ...
        p_rteqninfo%r_pressmax = MAX(p_rteqninfo%r_pressmax, r_pressure(i_dof))
        p_rteqninfo%r_pressmin = MIN(p_rteqninfo%r_pressmax, r_pressure(i_dof))
      END DO
...
!--- update timestep info structure
    ...
    p_rteqninfo%r_densmax = MAXVAL(r_Q(1,:))
    p_rteqninfo%r_densmin = MINVAL(r_Q(1,:))
