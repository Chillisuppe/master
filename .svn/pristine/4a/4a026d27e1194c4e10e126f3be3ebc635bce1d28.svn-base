...
!>        more balanced discretization of gravity source term
!>        In this case, the source term is discretized by enforcing a hydrostatic pressure
!>        and replacing the source term by its gradient. The hydrostatic pressure is calculated
!>        by \f$ P = mean(\rho e)*(\gamma-1) - (z-mean(z)) * mean(\rho) * GRAV \f$.
!>        The hydrostatic pressure gradient balances the pressure gradient in the flux/divflux
!>        but not up to machine precision (yet).
!
MODULE DG_equation
...
!--- array for generic IO
  ...
  INTERFACE pressure
    MODULE PROCEDURE pressure_scalar, pressure_array
  END INTERFACE

  INTERFACE gradpressure
    MODULE PROCEDURE gradpressure_scalar, gradpressure_array
  END INTERFACE
...
!
  SUBROUTINE source(r_rhs, i_faceunknowns, i_equadpts, r_eqwei, r_eMinvpsi, r_epsi, &
    ...
    INTEGER (KIND = GRID_SI)                                      :: i_quad, i_dof, i_maxloc
    REAL (KIND = GRID_SR), DIMENSION(i_equadpts)                  :: r_u_q, r_v_q, r_dphydx_q, &
      r_dphydy_q, r_pressure, r_exp, r_dexpdy, r_H, r_dHdy
    REAL (KIND = GRID_SR), DIMENSION(GRID_DIMENSION, i_equadpts)  :: r_coo_q
    ...
    REAL (KIND = GRID_SR), DIMENSION(i_faceunknowns)              :: r_press_hyd
    REAL (KIND = GRID_SR)                                         :: r_rho_m, r_z_m, r_nrg_m, r_nu, &
      r_nuaux, r_beta, r_alpha, r_sumweight

!--- interpolate to quadrature points and compute derivatives
    r_coo_q  = MATMUL(r_coo,   r_epsi)
    r_Q_q    = MATMUL(r_Qelmt, r_epsi)
    r_dQdx_q = MATMUL(r_Qelmt, r_ddxq)
    r_dQdy_q = MATMUL(r_Qelmt, r_ddyq)

! chandra: isothermal
!     r_pressure = pressure(r_Q_q)
!     r_exp    = EXP(r_coo_q(2,:) * GRID_GRAV * r_Q_q(1,:)/r_pressure)
!     r_dexpdy = MATMUL(EXP(-r_coo(2,:) * GRID_GRAV * r_Qelmt(1,:)/pressure(r_Qelmt)), r_ddyq)

! chandra: poly
!     r_pressure = pressure(r_Q_q)
!     r_nu    = p_equationparam%r_gamma
!     r_nuaux = r_nu / (r_nu - 1.0_GRID_SR)
!
!     r_beta   = MAXVAL(r_nuaux*r_pressure/r_Q_q(1,:)+GRID_GRAV*r_coo_q(2,:))
!     i_maxloc = MAXLOC(r_nuaux*r_pressure/r_Q_q(1,:)+GRID_GRAV*r_coo_q(2,:),1)
!     r_alpha  = r_pressure(i_maxloc)/r_Q_q(1,i_maxloc)**r_nu
!
!     r_H    = r_nuaux * LOG(r_alpha/r_nuaux *(r_beta - GRID_GRAV * r_coo_q(2,:)))
!     r_dHdy = MATMUL(r_H, r_ddyq)

!--- calculate mean values
    r_rho_m = SUM(r_Qelmt(1,:)) / i_faceunknowns
    r_nrg_m = SUM(r_Qelmt(4,:)) / i_faceunknowns
    r_z_m   = SUM(r_coo(2,:))   / i_faceunknowns

!--- calculate velocity
    r_u_q = velocity(r_Q_q(1,:), r_Q_q(2,:))
    r_v_q = velocity(r_Q_q(1,:), r_Q_q(3,:))

!--- calculate hydrostatic pressure and derivative in y-direction
    r_press_hyd = r_nrg_m*(p_equationparam%r_gamma-1.0_GRID_SR) - &
                  (r_coo_q(2,:)-r_z_m) * GRID_GRAV * r_rho_m

    r_sumweight = SUM(r_eqwei)
    r_dphydy_q  = MATMUL(r_press_hyd, r_ddyq) * r_sumweight

! chandra: poly
!     r_dphydx_q = 0.0_GRID_SR
!     r_dphydy_q = r_nuaux * r_Q_q(1,:) * (r_beta-r_coo_q(2,:)) * GRID_GRAV / &
!                  EXP(r_H) * r_dHdy!r_dQdy_q(4,:) * (p_equationparam%r_gamma - 1.0_GRID_SR)

! chandra: isothermal
!     r_dphydy_q = r_pressure * r_exp * r_dexpdy
...
!       r_source(2) = - r_dphydx_q(i_quad) * p_equationparam%r_gravityswitch
      r_source(3) = - r_dphydy_q(i_quad) * p_equationparam%r_gravityswitch
      r_source(4) = - r_dphydy_q(i_quad) * r_v_q(i_quad) * p_equationparam%r_gravityswitch
...
!--- pressure functions
!*******************************************************************************
! DESCRIPTION of [FUNCTION pressure_scalar]:
!> @brief computes the pressure
!
!> @param[in]       r_Q         state vector (rho, rho u, rho v, rho e)
!> @return                      return value
!
  FUNCTION pressure_scalar(r_Q) RESULT(r_pressure)
  ...
  END FUNCTION pressure_scalar

!*******************************************************************************
! DESCRIPTION of [FUNCTION pressure_array]:
!> @brief computes the pressure
!
!> @param[in]       r_Q         state vector (rho, rho u, rho v, rho e)
!> @return                      return value
!
  FUNCTION pressure_array(r_Q) RESULT(r_pressure)

    IMPLICIT NONE

    REAL (KIND = GRID_SR), DIMENSION(:,:), INTENT(IN) :: r_Q
    REAL (KIND = GRID_SR), DIMENSION(SIZE(r_Q(1,:)))            :: r_pressure, r_u, r_v
  ...
  END FUNCTION pressure_array

!--- gradpressure functions
!*******************************************************************************
! DESCRIPTION of [FUNCTION gradpressure_scalar]:
...
!> @return                          return value
!
  FUNCTION gradpressure_scalar(r_Q, r_dQdx, r_dQdy) RESULT(r_gradpressure)
...
!--- local declarations
  ...
  END FUNCTION gradpressure_scalar

!*******************************************************************************
! DESCRIPTION of [FUNCTION gradpressure_array]:
!> @brief computes the pressure gradient
!
!> @param[in]       r_Q             state vector \f$(\rho, \rho u, \rho v, \rho e)\f$
!> @param[in]       r_dQdx
...
!> @return                          return value
!
  FUNCTION gradpressure_array(r_Q, r_dQdx, r_dQdy) RESULT(r_gradpressure)

    IMPLICIT NONE

    REAL (KIND = GRID_SR), DIMENSION(:,:), INTENT(IN)   :: r_Q, r_dQdx, r_dQdy
    REAL (KIND = GRID_SR), DIMENSION(2,SIZE(r_Q(1,:)))  :: r_gradpressure

!--- local declarations
    REAL (KIND = GRID_SR), DIMENSION(SIZE(r_Q(1,:)))    :: r_u, r_v, r_velaux
    REAL (KIND = GRID_SR)                               :: r_coeff
  ...
  END FUNCTION gradpressure_array
