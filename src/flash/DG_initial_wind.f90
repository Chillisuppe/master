      ...
      DO i_elmt=1, i_numelmt
        DO i_dof=1, i_faceunknowns
          r_S(1,i_elmtdofs(i_dof,i_elmt)) = &
            (60._GRID_SR - r_coodof(1, i_elmtdofs(i_dof,i_elmt)) - r_coodof(2, i_elmtdofs(i_dof,i_elmt))) * GRID_GRAV
          r_S(2,i_elmtdofs(i_dof,i_elmt)) =  0.0_GRID_SR
          r_S(3,i_elmtdofs(i_dof,i_elmt)) =  0.8_GRID_SR ! * GRID_GRAV * 3.0
          r_S(4,i_elmtdofs(i_dof,i_elmt)) = -0.8_GRID_SR ! * GRID_GRAV * 3.0
        END DO
      END DO
    END IF
...
!--- loop over the DOFs
    dof_loop: DO i_count= 1, i_numelmt*GRID_femtypes%p_type(FEM_DG)%sig%i_unknowns
      ...
      r_Q(1, i_count) = (r_x + r_y + 80.0_GRID_SR) * GRID_GRAV
