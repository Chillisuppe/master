...
! 3. add nonhydrostatic terms		      v.hofmaier   01/2020
!
!*******************************************************************************
! MODULE DESCRIPTION:
...
MODULE DG_equation
  ...
  PUBLIC  i_nprogvars, i_nsrcterms, i_valQ, i_valS, &
          VAR3D_MZERO, VAR1D_HZERO, VAR1D_HCNST, VAR1D_PARACHECK, VAR1D_PCNST, &
          velocity, equation_initialize, divflux, flux, fluxGrav, source, &
          io_initialize, pre_timestepping, p_iovars

!***Angepasst
  INTEGER (KIND = GRID_SI), PARAMETER               :: i_nprogvars = 4 !< number of unknowns r_Q=(h,hu,hv,hw)
  INTEGER (KIND = GRID_SI), PARAMETER               :: i_nsrcterms = 2 !< number of grid source term variables r_S=(bathy,p^{nh})
...
!***Angepasst
  INTEGER (KIND = GRID_SI)                          :: VAR3D_MZERO     !< momentum variable
  INTEGER (KIND = GRID_SI)                          :: VAR1D_HZERO     !< height variable
  INTEGER (KIND = GRID_SI)                          :: VAR1D_HCNST     !< bathy variable
  INTEGER (KIND = GRID_SI)                          :: VAR1D_PCNST     !< bathy variable
...
!***Angepasst
    VAR3D_MZERO = grid_registerfemvar(i_femtype, i_length=3)
    VAR1D_HZERO = grid_registerfemvar(i_femtype)
    VAR1D_HCNST = grid_registerfemvar(i_femtype)

    i_valQ = (/ VAR1D_HZERO, VAR3D_MZERO, VAR3D_MZERO+1, VAR3D_MZERO+2 /)
    i_valS = (/ VAR1D_HCNST, VAR1D_PCNST /)
...
!>
!> @param[in]       r_Q         state vector (fluid depth, momentum in x-, momentum in y-, momentum in z-direction)
...
!
  FUNCTION flux(r_Q, r_S, r_iswet) RESULT(r_flux)
    ...
    REAL (KIND = GRID_SR)                                     :: r_u, r_v, r_w

!Alle Rechnungen angepasst
    r_u = velocity(r_Q(1), r_Q(2))
    r_v = velocity(r_Q(1), r_Q(3))
    r_w = velocity(r_Q(1), r_Q(4))

    r_flux(1,1) = r_Q(2)	!hu
    r_flux(1,2) = r_Q(3)	!hv

    r_flux(2,1) = r_Q(2)*r_u    !hu^2
    r_flux(2,2) = r_Q(2)*r_v    !huv

    r_flux(3,1) = r_Q(3)*r_u    !huv
    r_flux(3,2) = r_Q(3)*r_v    !hv^2

    r_flux(4,1) = r_Q(4)*r_u    !huw
    r_flux(4,2) = r_Q(4)*r_v    !hvw

    IF (.NOT. PRESENT(r_iswet) .OR. r_iswet==1.0) THEN
      r_flux(2,1) = r_flux(2,1) + 0.5_GRID_SR*r_Q(1)**2    !Was ist mit g? Bei mir steht h^2*g/2
      r_flux(3,2) = r_flux(3,2) + 0.5_GRID_SR*r_Q(1)**2    !Was ist mit g? Bei mir steht h^2*g/2
    END IF

    r_flux(2,1) = r_flux(2,1) + r_Q(1)*r_S(2)            !r_S(2)=p^{nh}, bisher unklar: Faktor 1/rho
    r_flux(3,2) = r_flux(3,2) + r_Q(1)*r_S(2)            
...
!
! Angepasst
! Wird nur im strong flux verwendet
!
!> @param[in]       r_Q         state vector (fluid depth, momentum in x-, momentum in y-, momentum in z-direction)
!> @param[in]       r_dQdx
!> @param[in]       r_dQdy
...
!--- local declarations
    REAL (KIND = GRID_SR)                                     :: r_u, r_v, r_w, r_wet
...
! Hier stop, weil ich die Ableitungsvectoren r_dQdx noch nicht angesehen habe. Muss ich r_dQdz erzeugen?
