...
MODULE DG_flux
  ...
  USE DG_equation, ONLY : i_nprogvars, i_nsrcterms, source, flux, fluxGrav, divflux, velocity
...
!
  SUBROUTINE fvm_flux(r_flux, r_Q, r_S, r_time, i_numelmt, i_numedge, i_faceunknowns, &
    ...
    REAL (KIND = GRID_SR)                                   :: r_edgelen, &
                                                               r_iswet, r_iswet_l, r_iswet_r, &
                                                               r_iswetg_l, r_iswetg_r
    ...
    REAL (KIND = GRID_SR), DIMENSION(i_faceunknowns)        :: r_h, r_b
    ...
    REAL (KIND = GRID_SR), DIMENSION(i_faceunknowns, i_faceunknowns) :: r_DxiDxi, r_DetaDeta, r_DxiDeta, &
                                                                     r_ddx, r_ddy, r_lap
    REAL (KIND = GRID_SR), DIMENSION(i_nprogvars,i_equadpts)      :: r_Q_q
    ...
    REAL (KIND = GRID_SR), DIMENSION(i_faceunknowns, i_equadpts)  :: r_eMinvdpsidx, r_eMinvdpsidy, &
...
!--- element loop for inner flux computation
    ...
    !$OMP r_Q_q, r_rhs, r_eMinvdpsidx, r_eMinvdpsidy, r_h, r_b, i_imaxh, r_iswet) &
    !$OMP SCHEDULE(STATIC)
...
!--- compute derivative operators
      ...
      r_eMinvdpsidx = r_eMinvdpsidxi *r_metrics_inv(1,1,i_elmt) + &
                      r_eMinvdpsideta*r_metrics_inv(1,2,i_elmt)
      r_eMinvdpsidy = r_eMinvdpsidxi *r_metrics_inv(2,1,i_elmt) + &
                      r_eMinvdpsideta*r_metrics_inv(2,2,i_elmt)
...
!--- compute unknowns at quadrature points
      ...
      r_h = r_Q(1,i_ledofs)
      r_b = r_S(1,i_ledofs)
      i_imaxh = MAXLOC(r_h + r_b, DIM=1)
      IF (MINVAL(r_h) < p_equationparam%r_wettol .AND. &
          r_h(i_imaxh)+r_b(i_imaxh)-MAXVAL(r_b) < p_equationparam%r_wettol) THEN
        r_iswet = 0.0_GRID_SR
      ELSE
        r_iswet = 1.0_GRID_SR
      END IF
...
!--- evaluate inner element flux and source term
      CALL elmtflux(r_rhs, i_faceunknowns, i_equadpts, r_eqwei, &
                    r_eMinvdpsidx, r_eMinvdpsidy, r_Q_q, r_iswet)
      CALL source(r_rhs, i_faceunknowns, i_equadpts, r_eqwei, r_eMinvpsi, r_epsi, &
                  r_ddxq, r_ddyq, r_lapq, r_Q(:,i_ledofs), r_S(:,i_ledofs), &
                  r_coodof(:,i_ledofs), r_time, r_iswet)
...
!--- edge loop for boundary flux computation
    ...
    !$OMP r_iswet_l, r_iswet_r, r_iswetg_l, r_iswetg_r, i_imaxh) &
    !$OMP SCHEDULE(STATIC)
    ...
    IF((i_edgeboundary(i_edge) <= 0) .OR. (i_edgeboundary(i_edge) == i_edge)) THEN
      ...
      i_imaxh = MAXLOC(r_Q_l(1,:)+r_S_l(1,:), DIM=1)
      IF (MINVAL(r_Q_l(1,:)) < p_equationparam%r_wettol .AND. &
          r_Q_l(1,i_imaxh)+r_S_l(1,i_imaxh)-MAXVAL(r_S_l(1,:)) < p_equationparam%r_wettol) THEN
        r_iswet_l = 0.0_GRID_SR
      ELSE
        r_iswet_l = 1.0_GRID_SR
      END IF

      i_imaxh = MAXLOC(r_Q_r(1,:)+r_S_r(1,:), DIM=1)
      IF (MINVAL(r_Q_r(1,:)) < p_equationparam%r_wettol .AND. &
          r_Q_r(1,i_imaxh)+r_S_r(1,i_imaxh)-MAXVAL(r_S_r(1,:)) < p_equationparam%r_wettol) THEN
        r_iswet_r = 0.0_GRID_SR
      ELSE
        r_iswet_r = 1.0_GRID_SR
      END IF

      IF (MAXVAL(r_Q_l(1,i_gpsiidx)) < p_equationparam%r_wettol) THEN
        r_iswetg_l = 0.0_GRID_SR
      ELSE
        r_iswetg_l = 1.0_GRID_SR
      END IF

      IF (MAXVAL(r_Q_r(1,i_gpsiidx)) < p_equationparam%r_wettol) THEN
        r_iswetg_r = 0.0_GRID_SR
      ELSE
        r_iswetg_r = 1.0_GRID_SR
      END IF

      CALL edgeflux(r_rhs_l, r_rhs_r, r_normal, r_iswet_l, r_iswet_r, r_iswetg_l, r_iswetg_r, &
...
!> @param[in]     r_eMinvdpsidx
!> @param[in]     r_eMinvdpsidy
...
!> @param[in]     r_iswet
!
  SUBROUTINE elmtflux(r_rhs, i_faceunknowns, i_equadpts, r_eqwei, &
                      r_eMinvdpsidx, r_eMinvdpsidy, r_Q, r_iswet)
    ...
    REAL (KIND = GRID_SR), DIMENSION(:,:), INTENT(in)       :: r_Q, r_eMinvdpsidx, r_eMinvdpsidy
    REAL (KIND = GRID_SR), INTENT(in)                       :: r_iswet

    INTEGER (KIND = GRID_SI)                                :: i_quad, i_dof
    REAL (KIND = GRID_SR), DIMENSION(i_nprogvars,2)         :: r_Div
...
!--- compute flux divergence
      r_Div = flux(r_Q(:,i_quad), r_iswet=r_iswet)

!--- multiply with r_eqwei*Minv*psi for each dof
      elmt_dof_loop: DO i_dof=1,i_faceunknowns
        r_rhs(:,i_dof) = r_rhs(:,i_dof) + &
          r_eqwei(i_quad)*(r_eMinvdpsidx(i_dof,i_quad)*r_Div(:,1)+r_eMinvdpsidy(i_dof,i_quad)*r_Div(:,2))
...
!>
!> @param[out]    r_rhs_l         edge flux for left element; DIMENSION(3,i_faceunknowns)
!> @param[out]    r_rhs_r         edge flux for right element; DIMENSION(3,i_faceunknowns)
...
!
  SUBROUTINE edgeflux(r_rhs_l, r_rhs_r, r_normal, r_iswet_l, r_iswet_r, r_iswetg_l, r_iswetg_r, &
                      i_faceunknowns, i_gquadpts, r_gqwei, r_gMinvpsi, &
    ...
    REAL (KIND = GRID_SR), INTENT(in)                       :: r_iswet_l, r_iswet_r, &
                                                               r_iswetg_l, r_iswetg_r
...
!--- local declarations
    INTEGER (KIND = GRID_SI)                                :: i_quad, i_dof
    REAL (KIND = GRID_SR)                                   :: r_h_l_star, r_hu_l_star, r_hv_l_star, &
                                                               r_h_r_star, r_hu_r_star, r_hv_r_star, &
                                                               r_u, r_v, r_un, r_ut
    ...
    IF (MAXVAL(r_Q_l(1,:)) > p_equationparam%r_wettol .OR. MAXVAL(r_Q_r(1,:)) > p_equationparam%r_wettol) THEN

!--- perform edge quadrature
      edge_quad_loop: DO i_quad=1,i_gquadpts

        IF (r_Q_r(1,i_quad) <= p_equationparam%r_wettol .AND. r_Q_l(1,i_quad)+r_S_l(1,i_quad) < r_S_r(1,i_quad)) THEN
          ! right side dry, left side wet, wall case
          r_h_l_star  = r_Q_l(1,i_quad)
          r_hu_l_star = r_Q_l(2,i_quad)
          r_hv_l_star = r_Q_l(3,i_quad)

          r_u  = velocity(r_Q_l(1,i_quad), r_Q_l(2,i_quad))
          r_v  = velocity(r_Q_l(1,i_quad), r_Q_l(3,i_quad))
          r_un =  r_normal(1)*r_u + r_normal(2)*r_v
          r_ut = -r_normal(2)*r_u + r_normal(1)*r_v

          r_h_r_star  = r_Q_l(1,i_quad)
          r_hu_r_star = r_h_r_star*(-r_ut*r_normal(2) - r_un*r_normal(1))
          r_hv_r_star = r_h_r_star*( r_ut*r_normal(1) - r_un*r_normal(2))

          r_Fstar = riemannsolver([r_h_l_star, r_hu_l_star, r_hv_l_star], &
                                  [r_h_r_star, r_hu_r_star, r_hv_r_star], &
                                  r_normal)
          r_Fleft = 0.0_GRID_SR
          r_Frght = r_Fstar

        ELSE IF (r_Q_l(1,i_quad) <= p_equationparam%r_wettol .AND. r_Q_r(1,i_quad)+r_S_r(1,i_quad) < r_S_l(1,i_quad)) THEN
          ! left side dry, right side wet, wall case
          r_h_r_star  = r_Q_r(1,i_quad)
          r_hu_r_star = r_Q_r(2,i_quad)
          r_hv_r_star = r_Q_r(3,i_quad)

          r_u  = velocity(r_Q_r(1,i_quad), r_Q_r(2,i_quad))
          r_v  = velocity(r_Q_r(1,i_quad), r_Q_r(3,i_quad))
          r_un =  r_normal(1)*r_u + r_normal(2)*r_v
          r_ut = -r_normal(2)*r_u + r_normal(1)*r_v

          r_h_l_star  = r_Q_r(1,i_quad)
          r_hu_l_star = r_h_l_star*(-r_ut*r_normal(2) - r_un*r_normal(1))
          r_hv_l_star = r_h_l_star*( r_ut*r_normal(1) - r_un*r_normal(2))
        ...
        ELSE
!--- fix for discontinuous bathymetry according to Audusse et al. (2004),
!--- Xing, Zhang & Shu (2010), Xing & Zhang (2013)
          r_h_l_star  = MAX(0._GRID_SR, r_Q_l(1,i_quad) + r_S_l(1,i_quad) - &
                            MAX(r_S_l(1,i_quad), r_S_r(1,i_quad)))
          r_hu_l_star = r_h_l_star * velocity(r_Q_l(1,i_quad), r_Q_l(2,i_quad))
          r_hv_l_star = r_h_l_star * velocity(r_Q_l(1,i_quad), r_Q_l(3,i_quad))
          r_h_r_star  = MAX(0._GRID_SR, r_Q_r(1,i_quad) + r_S_r(1,i_quad) - &
                            MAX(r_S_l(1,i_quad), r_S_r(1,i_quad)))
          r_hu_r_star = r_h_r_star * velocity(r_Q_r(1,i_quad), r_Q_r(2,i_quad))
          r_hv_r_star = r_h_r_star * velocity(r_Q_r(1,i_quad), r_Q_r(3,i_quad))

          r_Fstar = riemannsolver([r_h_l_star, r_hu_l_star, r_hv_l_star], &
                                  [r_h_r_star, r_hu_r_star, r_hv_r_star], &
                                  r_normal)

          r_Fleft = (1.0-r_iswet_l)*MATMUL(fluxGrav(r_Q_l(1,i_quad)), r_normal) + &
            MATMUL(fluxGrav(r_h_l_star) - fluxGrav(r_Q_l(1,i_quad)), r_normal)
          r_Frght = (1.0-r_iswet_r)*MATMUL(fluxGrav(r_Q_r(1,i_quad)), r_normal) + &
            MATMUL(fluxGrav(r_h_r_star) - fluxGrav(r_Q_r(1,i_quad)), r_normal)
        END IF
