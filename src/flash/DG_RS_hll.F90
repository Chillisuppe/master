...
!> @brief HLL Riemann solver for shallow water equations
!>
...
!--- compute HLL flux
      ...
      ELSEIF ((r_S_l < 0.0_GRID_SR) .AND. (r_S_r >= 0.0_GRID_SR)) THEN
        r_flux_hll = (r_S_r*r_flux_l(:,1) - r_S_l*r_flux_r(:,1) + &
                      r_S_l*r_S_r*(r_Q_rot_r-r_Q_rot_l)) / (r_S_r - r_S_l)
