...
!>
!> @param[in]     r_Q_l           left state vector (fluid depth, momentum in x-, momentum in y-, momentum in z-direction)
!> @param[in]     r_Q_r           right state vector (fluid depth, momentum in x-, momentum in y-, momentum in z-direction)
...
!--- compute velocities and their normal components
    ...
    r_w_l = velocity(r_Q_l(1), r_Q_l(4))
    r_u_r = velocity(r_Q_r(1), r_Q_r(2))
    r_v_r = velocity(r_Q_r(1), r_Q_r(3))
    r_w_r = velocity(r_Q_r(1), r_Q_r(4))
...
!--- compute q_r - q_l as three-dimensional vector
    ...
    r_flux_help = r_lambda*MATMUL(RESHAPE(r_dq, [4, 1]), RESHAPE(r_normal, [1, GRID_DIMENSION]))
