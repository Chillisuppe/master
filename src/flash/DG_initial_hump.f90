!*******************************************************************************
!
!> @file  DG_initial_hump.f90
!> @brief contains module DG_initial
!
!*******************************************************************************
! MODULE DESCRIPTION:
!> @brief initialize fields with a Gaussian hump
...
!
  SUBROUTINE initial_setparam
    ...
    p_testparam%treal(1)%c_keyword = 'BUMP_CENTERPOINT'
    p_testparam%treal(1)%i_size    = 2

    p_testparam%treal(2)%c_keyword = 'BUMP_RADIUS'
...
!--- local declarations
    ...
    REAL (KIND = GRID_SR)                                 :: r_rm, r_rm2, r_r2, &
      r_hgt
...
!--- initialize some constants
    ...
    r_rm    = p_testparam%treal(2)%p_value(1)
    r_rm2   = r_rm * r_rm
    r_hgt   = p_testparam%treal(3)%p_value(1)

!--- loop over the DOFs
    DO i_count=1,i_num
      r_Q(1,i_count) = p_equationparam%r_depth * GRID_GRAV
      r_tmp(:)         = r_coodof(:,i_count) - r_centr(:)
      r_r2             = dot_product(r_tmp, r_tmp)
      IF(r_r2 < r_rm2) THEN
        r_Q(1,i_count) = r_Q(1,i_count) + &
          r_hgt * (0.5_GRID_SR * (COS(SQRT(r_r2)/r_rm*GRID_PI)+1.0_GRID_SR)) * GRID_GRAV
      END IF
