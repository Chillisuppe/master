...
!>        This equation set uses potential temperature as energy representation
!
MODULE DG_equation
  ...
  PUBLIC  i_nprogvars, i_nsrcterms, i_valQ, i_valS, &
          VAR1D_RHO, VAR2D_MOM, VAR1D_PTE, VAR1D_PARACHECK, &
...
!--- constants for the DG variables
  ...
  INTEGER (KIND = GRID_SI)                          :: VAR1D_PTE       !< potential energy variable
...
!--- total variables
    ...
    VAR1D_PTE = grid_registerfemvar(i_femtype)

    i_valQ  = (/ VAR1D_RHO, VAR2D_MOM, VAR2D_MOM+1, VAR1D_PTE /)
!     i_valS  = (/ /)
...
!>
!> @param[in]       r_Q         state vector (rho, rho u, rho v, rho theta)
...
!
  FUNCTION flux(r_Q, r_S) RESULT(r_flux)
    ...
    r_flux(4,1) = r_Q(4)*r_u
    r_flux(4,2) = r_Q(4)*r_v
...
!--- interpolate to quadrature points and compute derivatives
    ...
    r_rhou_q = MATMUL(r_Qelmt(2,:), r_epsi)
    r_rhov_q = MATMUL(r_Qelmt(3,:), r_epsi)
...
!--- local declarations
    ...
    r_div_tmp(4,1) = (- r_u*r_Q(4)*r_dQdx(1) + (r_Q(4)*r_dQdx(2)+r_Q(2)*r_dQdx(4)) ) /r_Q(1)
    r_div_tmp(4,2) = (- r_v*r_Q(4)*r_dQdy(1) + (r_Q(4)*r_dQdy(3)+r_Q(3)*r_dQdy(4)) ) /r_Q(1)
...
!
  FUNCTION pressure(r_Q) RESULT(r_pressure)
    ...
    REAL (KIND = GRID_SR), DIMENSION(i_nprogvars), INTENT(IN) :: r_Q
    REAL (KIND = GRID_SR)                                     :: r_pressure

    r_pressure = p_equationparam%r_pref * &
      (r_Q(4)*p_equationparam%r_rgas/p_equationparam%r_pref)**(p_equationparam%r_gamma)
...
!
!> @param[in]       r_Q             state vector \f$(\rho, \rho u, \rho v, \rho \theta)\f$
!> @param[in]       r_dQdx
!> @param[in]       r_dQdy
!> @return                          return value
!
...
!--- local declarations
    ...
    r_coeff = p_equationparam%r_pref * p_equationparam%r_gamma * &
      (p_equationparam%r_rgas/p_equationparam%r_pref)**(p_equationparam%r_gamma)

    r_gradpressure(1) = r_coeff * r_Q(4)**(p_equationparam%r_gamma-1.0_GRID_SR) * r_dQdx(4)
    r_gradpressure(2) = r_coeff * r_Q(4)**(p_equationparam%r_gamma-1.0_GRID_SR) * r_dQdy(4)
...
!--- the pot. temp. formula is rho*theta = P0/R * (P0/P)^gamma (see Giraldo & Restelli 2008)
    r_Q(1) = r_Qprim(1)
    r_Q(2) = r_Qprim(1) * r_Qprim(2)
    r_Q(3) = r_Qprim(1) * r_Qprim(3)
    r_Q(4) = p_equationparam%r_pref / p_equationparam%r_rgas * &
      (r_Qprim(4)/p_equationparam%r_pref)**(1.0_GRID_SR/p_equationparam%r_gamma)
...
!--- potential temperature
    ...
    p_iovars(4)%c_varname       = 'pot_temp'
    p_iovars(4)%c_long_name     = 'potential temperature'
    p_iovars(4)%c_standard_name = 'potential_temperature' ! not CF conform!
...
!--- exact potential temperature
    p_iovars(8)%r_factor        = 1.0_GRID_SR
    p_iovars(8)%c_varname       = 'exact_pot_temp'
    p_iovars(8)%c_long_name     = 'potential temperature'
    p_iovars(8)%c_standard_name = 'exact_potential_temperature' ! not CF conform!
