...
!
  SUBROUTINE limiter(p_ghand, r_Q, i_elementedges, i_edgeinfo, r_edgelength, r_metrics_inv, &
    ...
    REAL(KIND = GRID_SR), DIMENSION(3)                        :: r_Delta_hat
    REAL(KIND = GRID_SR), DIMENSION(i_nprogvars)              :: r_Deltaphibar, &
                                                                 r_phitilde, r_Deltaphi1, r_Deltaphi2
    REAL(KIND = GRID_SR), DIMENSION(2,2)                      :: r_matrix
    REAL(KIND = GRID_SR), DIMENSION(i_nprogvars,3)            :: r_Delta
...
!--- allocate workspace
    ALLOCATE(r_vander_m(i_faceunknowns, 3), &
             ...
             r_emean(i_nprogvars, i_numelmt), r_dphi(i_nprogvars, i_numedge), stat=i_alct)
...
!--- compute mean values and barycenter for each element
    DO i_elmt=1, i_numelmt
      ...
      r_emean(4,i_elmt)= DOT_PRODUCT(r_evander(:,1), r_Q(4,i_elementdofs(:,i_elmt))) / SUM(r_evander(:,1))
...
!--- compute coordinates of edge centres
    DO i_edge=1, i_numedge
      ...
      ELSE
        ...
        r_dphi(1,i_edge) = DOT_PRODUCT(r_vander_m(:,i_gloc) , r_Q(1,i_elementdofs(:,i_elmt))) - r_emean(1,i_elmt)
        ...
        r_dphi(4,i_edge) = DOT_PRODUCT(r_vander_m(:,i_gloc) , r_Q(4,i_elementdofs(:,i_elmt))) - r_emean(4,i_elmt)
      END IF
    END DO
...
!--- compute r_phitilde for local edge
        r_phitilde(1) = DOT_PRODUCT(r_vander_m(:,i_gloc) , r_Q(1,i_elementdofs(:,i_elmt)))  - r_emean(1,i_elmt)
        ...
        r_phitilde(4) = DOT_PRODUCT(r_vander_m(:,i_gloc) , r_Q(4,i_elementdofs(:,i_elmt))) - r_emean(4,i_elmt)
...
!--- compute Delta_i
        ...
        r_Delta(4,i_gloc) = TVB_minmod(r_phitilde(4), r_nu*r_Deltaphibar(4), r_MM, r_deltax)

      END DO edge_loop

!--- IF sum Delta_i = ...
      DO i_eqn=1, 3!i_nprogvars
        IF (ABS(SUM(r_Delta(i_eqn,:))) <= 1E-10) THEN
          r_Q(i_eqn,i_elementdofs(:,i_elmt)) = r_emean(i_eqn,i_elmt) + MATMUL(r_basis, r_Delta(i_eqn,:))
        ELSE

          r_pos = MAX(0._GRID_SR, r_Delta(i_eqn,1)) + MAX(0._GRID_SR, r_Delta(i_eqn,2)) + MAX(0._GRID_SR, r_Delta(i_eqn,3)) !+ MAX(0._GRID_SR, r_Delta(i_eqn,4))
          r_neg = MAX(0._GRID_SR,-r_Delta(i_eqn,1)) + MAX(0._GRID_SR,-r_Delta(i_eqn,2)) + MAX(0._GRID_SR,-r_Delta(i_eqn,3)) !+ MAX(0._GRID_SR,-r_Delta(i_eqn,4))
