!*******************************************************************************
!
!> @file  DG_initial_conicalisland-ic.f90
...
!
  SUBROUTINE initial_setparam
    ...
    p_testparam%treal(2)%c_keyword = 'PARAM_X0'
...
!--- local declarations
    INTEGER (KIND = GRID_SI)                                :: i_count, i_elmt, &
      i_faceunknowns
    REAL (KIND = GRID_SR)                                   :: r_a, r_K, r_x0, r_hgt

    r_a  = p_testparam%treal(1)%p_value(1)
    r_x0 = p_testparam%treal(2)%p_value(1)

    r_K = SQRT(0.75_GRID_SR*r_a/p_equationparam%r_depth**3)
...
!--- initialize some constants
    ...
    DO i_elmt=1, i_numelmt
      DO i_count=1, i_faceunknowns
        r_hgt = r_a / COSH(r_K * (r_x0 - r_coodof(1,i_elmtdofs(i_count,i_elmt))))**2
        r_Q(1,i_elmtdofs(i_count,i_elmt)) = &
          MAX(0.0_GRID_SR, (p_equationparam%r_depth+r_hgt)*GRID_GRAV - r_S(1,i_elmtdofs(i_count,i_elmt)))
        r_Q(2,i_elmtdofs(i_count,i_elmt)) = r_Q(1,i_elmtdofs(i_count,i_elmt)) * 2.0_GRID_SR * &
          (SQRT(r_Q(1,i_elmtdofs(i_count,i_elmt))) - &
           SQRT(MAX(0.0_GRID_SR, p_equationparam%r_depth*GRID_GRAV - r_S(1,i_elmtdofs(i_count,i_elmt)))))
